...
//User register and login
   ...
   public function userLogin(request $request){
       if(Auth::attempt(['email'=>$request->input('email'),'password'=>$request->input('password')])){
           $user=Auth::user();
         //  $response['token']=$user->createToken('AJITH')->plainTextToken;
         //  $response['name']=$user->name;
           return response()->json('login success',200);}
           
       else{

           return response()->json('Invalid Username and Password',401);
       }
...
public function findsim(){
    $data=DB::select("SELECT sims.* FROM sims INNER JOIN customers ON sims.name= customers.name");
